...
def drifting_loss(
    ...
    loss += 3*loss_over_feature_lists(gen, pos)
    return loss
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_dir', type=str, default='drift-circles', help='Directory to save models')
    parser.add_argument('--mae_ckpt', type=str, default='saved_runs/mae/mae-circles/checkpoints/best_mae.tar', help='Path to a MAE checkpoint to load')
    ...
    mae = MAE(in_ch_pixel=1, spatial_downsample=[8, 2, 2]).to(device)
    ...
    model = JiTUncond_B_16(in_channels=1, input_size=128).to(device)
    ...
    for epoch in pbar:
        ...
        with torch.no_grad():
            ...
            for i in range(4):
                # Handle grayscale: [C, H, W] -> [H, W]
                img = pred[i].squeeze().cpu().numpy()
                sample_axes[i].imshow(img, cmap="Blues")
